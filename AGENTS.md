...
- Maintain `AlexbeatsZ/AstrBot` as the user's fork. Push changes to the fork; do not open an upstream PR unless explicitly requested.
- Agy runs from a persistent AstrBot `data` volume with an independent HOME, UMO-scoped workspace, custom agent, native terminal sandbox, and a small host-tool allowlist. Do not use `--dangerously-skip-permissions` as the normal execution path.
- Only portable Agy Skills may be synchronized directly. Skills that depend on AstrBot tools require the controlled host-tool bridge or a later MCP design.
- Server deployment uses a fixed custom image. Preserve `Sinm` as disabled and verify the `astrbot_network`, Dashboard, Agy provider, default provider, Playwright, and OpenClaw health after rebuilding.

## Local active work

- Design Agy-native Skills management while preserving the sandbox, workspace isolation, and host-tool allowlist boundaries.
