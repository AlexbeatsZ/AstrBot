...
Agy uses its own local login state and native tools. AstrBot remains responsible for conversation history, so every invocation is stateless and does not use `--continue` or `--conversation`.
...
- Agy decides when to use its native tools; it does not emit AstrBot Function Calling requests.
