...
/**
 * Detect the managed or external Agy CLI installation
 */
export const getAgyCliStatus = <ThrowOnError extends boolean = false>(options: OptionsLegacyParser<GetAgyCliStatusData, ThrowOnError>) => {
    return (options?.client ?? client).post<GetAgyCliStatusResponse, GetAgyCliStatusError, ThrowOnError>({
        ...options,
        url: '/api/v1/providers/agy-cli/status'
    });
};

/**
 * Install or update the managed Agy CLI binary
 */
export const installOrUpdateAgyCli = <ThrowOnError extends boolean = false>(options: OptionsLegacyParser<InstallOrUpdateAgyCliData, ThrowOnError>) => {
    return (options?.client ?? client).post<InstallOrUpdateAgyCliResponse, InstallOrUpdateAgyCliError, ThrowOnError>({
        ...options,
        url: '/api/v1/providers/agy-cli/install'
    });
};
