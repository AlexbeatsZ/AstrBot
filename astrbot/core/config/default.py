        ...
        "metadata": {
            "provider": {
                ...
                "items": {
                    ...
                    "agy_command": {
                        "description": "Agy 可执行文件",
                        "type": "string",
                        "hint": "默认使用供应商页面安装到 data/agy/bin 的托管版本；也可填写自定义命令或绝对路径。",
                    },
                    "agy_working_directory": {
                        "description": "Agy 工作目录",
                        "type": "string",
                        "hint": "留空时使用 AstrBot 数据工作区目录。默认会在此目录下为每个聊天来源创建独立子目录。",
                    },
                    "agy_isolate_workspaces": {
                        "description": "隔离 Agy 聊天工作区",
                        "type": "bool",
                        "hint": "为每个聊天来源使用独立工作目录，避免不同会话通过 Agy 文件工具互相读取。",
                    },
                    "agy_sandbox": {
                        "description": "启用 Agy 原生沙箱",
                        "type": "bool",
                        "hint": "向 Agy 传递 --sandbox，并使用 AstrBot 专用 HOME 中的 proceed-in-sandbox 安全策略；不会创建额外 Docker 容器。",
                    },
                    "agy_print_timeout": {
                        "description": "Agy 打印模式超时",
                        "type": "string",
                        "hint": "传给 agy --print-timeout 的持续时间，例如 10m。",
                    },
                    "agy_thinking_level": {
                        "description": "Agy 思考级别",
                        "type": "string",
                        "options": [
                            "adaptive",
                            "off",
                    ...
                    "agy_system_prompt_mode": {
                        "description": "Agy 系统提示词模式",
                        "type": "string",
                        "options": ["filtered", "full", "none"],
                        "hint": "filtered 会移除 AstrBot 专用工具和技能说明，避免与 agy 原生工具冲突。",
                    },
                    "agy_dangerously_skip_permissions": {
                        "description": "自动批准 Agy 工具权限",
                        "type": "bool",
                        "hint": "向 agy 传递 --dangerously-skip-permissions。仅在信任所有聊天用户和 agy 工作目录时启用。",
                    },
                    "agy_host_tool_allowlist": {
                        "description": "Agy 宿主工具白名单",
                        "type": "list",
                        "items": {"type": "string"},
                        "hint": "仅向 Agy 暴露这些 AstrBot 工具。工具在 Agy 沙箱之外执行，默认只开放渲染和图片发送工具。",
                    },
                    "agy_max_output_bytes": {
                        "description": "Agy 最大输出字节数",
                        "type": "int",
                    },
                    "agy_env": {
                        "description": "Agy 环境变量",
                        "type": "dict",
                        "items": {},
                        "hint": "仅添加到 agy 子进程；代理也可以使用提供商的代理地址字段。",
                    },
